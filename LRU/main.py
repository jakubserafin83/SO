...
frames = np.full((nof, 2), [-1]) #setting default value of empty frame/default age to -1

temp_queue = np.empty((0, 2), dtype=int) #queue declaration
...
while t_unit <= nop:
    ...
    if queue.size > 0:
        ...
        if np.all(frames[:, 0] != queue[0, 1]): #if first queue page isn't already in memory
        ...
        else: #if incoming page is in the memory, reset age value to 0
            hit_rate.write(str(0) + '\n')
            for i in range(nof):
                if frames[i, 0] == queue[0,1]:
                    frames[i,1] = 0

        queue = np.delete(queue, 0, axis=0) #deletes least page from queue
        print(f'frames | age \n{frames} \n####')
        print(f'queue \n{queue} \n####')
